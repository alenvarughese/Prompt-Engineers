...
export const SkillRadarChart = ({ current = {}, target = {}, isDarkMode }) => {
  // 1. Get all unique skill names from both lists
  const currentKeys = Object.keys(current);
  const targetKeys = Object.keys(target);
  const allSubjects = Array.from(new Set([...currentKeys, ...targetKeys]));

  // 2. Map data, defaulting to 0 if a skill is missing in one list
  const data = allSubjects.map(subject => ({
    subject,
    A: current[subject] || 0, // Current Level
    B: target[subject] || 0,  // Target Level
    fullMark: 100
  }));

  // 3. Fallback if data is empty (prevents white screen)
  if (data.length === 0) {
    return (
      <div className="flex items-center justify-center h-full text-gray-400 text-sm">
        No skill data available to visualize.
      </div>
    );
  }
